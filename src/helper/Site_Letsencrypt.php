...
class Site_Letsencrypt {
	...
	function __construct() {
		$this->conf_dir = EE_ROOT_DIR . '/acme-conf';
		$this->setRepository();
		$this->setAcmeClient();
	}
	...
	public function authorize( Array $domains, $wildcard = false ) {
		...
		foreach ( $authorizationChallengesToSolve as $authorizationChallenge ) {
			...
			if ( ! $wildcard ) {
				...
				$fs->copy( SITE_TEMPLATE_ROOT . '/vhost.d_default_letsencrypt.mustache', EE_ROOT_DIR . '/nginx/vhost.d/default' );
				$challange_dir = EE_ROOT_DIR . '/nginx/html/.well-known/acme-challenge';
	...
	private function moveCertsToNginxProxy( string $domain ) {
		...
		$key_dest_file   = EE_ROOT_DIR . '/nginx/certs/' . $domain . '.key';
		$crt_dest_file   = EE_ROOT_DIR . '/nginx/certs/' . $domain . '.crt';
		$chain_dest_file = EE_ROOT_DIR . '/nginx/certs/' . $domain . '.chain.pem';
	...
	public function cleanup() {
		...
		$challange_dir = EE_ROOT_DIR . '/nginx/html/.well-known';
		$challange_rule_file = EE_ROOT_DIR . '/nginx/vhost.d/default';
